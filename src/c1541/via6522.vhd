...
architecture Gideon of via6522 is
    ...
    constant latch_reset_pattern : unsigned(15 downto 0) := X"5550";
    ...
    signal timer_a_latch : unsigned(15 downto 0) := latch_reset_pattern;
    signal timer_b_latch : unsigned(15 downto 0) := latch_reset_pattern;
    signal timer_a_count : unsigned(15 downto 0) := latch_reset_pattern;
    signal timer_b_count : unsigned(15 downto 0) := latch_reset_pattern;
        ...
        if rising_edge(clock) then            
            ...
            if wen='1' and falling = '1' then
                ...
                when X"4" => -- TA LO counter (write=latch)
                    timer_a_latch(7 downto 0) <= unsigned(data_in);
                    
                when X"5" => -- TA HI counter
                    timer_a_latch(15 downto 8) <= unsigned(data_in);
                    timer_a_flag <= '0';
                ...
                when X"8" => -- TB LO latch
                    timer_b_latch(7 downto 0) <= unsigned(data_in);
            ...
            when X"4" => -- TA LO counter
                data_out <= std_logic_vector(timer_a_count(7 downto 0));
            when X"5" => -- TA HI counter
                data_out <= std_logic_vector(timer_a_count(15 downto 8));
            when X"6" => -- TA LO latch
                data_out <= std_logic_vector(timer_a_latch(7 downto 0));
            when X"7" => -- TA HI latch
                data_out <= std_logic_vector(timer_a_latch(15 downto 8));
            when X"8" => -- TA LO counter
                data_out <= std_logic_vector(timer_b_count(7 downto 0));
            when X"9" => -- TA HI counter
                data_out <= std_logic_vector(timer_b_count(15 downto 8));
            when X"A" => -- SR
                data_out  <= shift_reg;
            when X"B" => -- ACR
                data_out  <= acr;
        ...
        begin
            if rising_edge(clock) then
                if falling = '1' then
                    ...
                    if timer_a_reload = '1' then
                        ...
                        if write_t1c_l = '1' then
                            timer_a_count(7 downto 0) <= unsigned(data_in);
                        end if;
                        timer_a_reload <= '0';
                ...
                if write_t1c_h = '1' then
                    ...
                    timer_a_count  <= unsigned(data_in) & timer_a_latch(7 downto 0);
                    timer_a_reload <= '0';
                end if;
            ...
            if rising_edge(clock) then
                ...
                if falling = '1' then
                    ...
                    if timer_b_decrement = '1' then
                        if timer_b_count = 0 then
                            if timer_b_oneshot_trig = '1' then
                                timer_b_oneshot_trig <= '0';
                                timer_b_timeout <= '1';
                            end if;
                        end if;
                        if timer_b_count(7 downto 0) = 0 then
                ...
                if write_t2c_h = '1' then
                    timer_b_count <= unsigned(data_in) & timer_b_latch(7 downto 0);
